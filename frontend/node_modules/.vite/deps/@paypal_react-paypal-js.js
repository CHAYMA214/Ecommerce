...
var validateBraintreeAuthorizationData = function(scriptContext) {
	var _a, _b;
	if (!((_a = scriptContext === null || scriptContext === void 0 ? void 0 : scriptContext.options) === null || _a === void 0 ? void 0 : _a[SDK_SETTINGS.DATA_CLIENT_TOKEN]) && !((_b = scriptContext === null || scriptContext === void 0 ? void 0 : scriptContext.options) === null || _b === void 0 ? void 0 : _b[SDK_SETTINGS.DATA_USER_ID_TOKEN])) throw new Error(EMPTY_BRAINTREE_AUTHORIZATION_ERROR_MESSAGE);
...
var isValidBraintreeNamespace = function(braintreeSource) {
	var _a, _b;
	if (typeof ((_a = braintreeSource === null || braintreeSource === void 0 ? void 0 : braintreeSource.client) === null || _a === void 0 ? void 0 : _a.create) !== "function" && typeof ((_b = braintreeSource === null || braintreeSource === void 0 ? void 0 : braintreeSource.paypalCheckout) === null || _b === void 0 ? void 0 : _b.create) !== "function") throw new Error("The braintreeNamespace property is not a valid BraintreeNamespace type.");
	return true;
};
/**
...
var decorateActions = function(buttonProps, payPalCheckoutInstance) {
	...
	if (typeof createOrderRef === "function") buttonProps.createOrder = function(data, actions) {
		return createOrderRef(data, __assign(__assign({}, actions), { braintree: payPalCheckoutInstance }));
	};
	if (typeof createBillingAgreementRef === "function") buttonProps.createBillingAgreement = function(data, actions) {
		return createBillingAgreementRef(data, __assign(__assign({}, actions), { braintree: payPalCheckoutInstance }));
	};
	if (typeof onApproveRef === "function") buttonProps.onApprove = function(data, actions) {
		return onApproveRef(data, __assign(__assign({}, actions), { braintree: payPalCheckoutInstance }));
	};
...
var getBraintreeNamespace = function(braintreeSource) {
	if (braintreeSource && isValidBraintreeNamespace(braintreeSource)) return Promise.resolve(braintreeSource);
	return Promise.all([loadCustomScript({ url: BRAINTREE_SOURCE }), loadCustomScript({ url: BRAINTREE_PAYPAL_CHECKOUT_SOURCE })]).then(function() {
...
var BraintreePayPalButtons = function(_a) {
	...
	(0, import_react.useEffect)(function() {
		getBraintreeNamespace(braintreeNamespace).then(function(braintree) {
			...
			return braintree.client.create({ authorization: clientTokenizationKey || clientToken }).then(function(clientInstance) {
				var merchantProp = merchantAccountId ? { merchantAccountId } : {};
				return braintree.paypalCheckout.create(__assign(__assign({}, merchantProp), { client: clientInstance }));
	...
	return import_react.createElement(import_react.Fragment, null, providerContext.braintreePayPalCheckoutInstance && import_react.createElement(PayPalButtons, __assign({
		className,
		disabled,
		forceReRender
...
var generateMissingHostedFieldsError = function(_a) {
	var _b = _a.components, components = _b === void 0 ? "" : _b, _d = _a[SDK_SETTINGS.DATA_NAMESPACE], dataNamespace = _d === void 0 ? DEFAULT_PAYPAL_NAMESPACE : _d;
...
var hasDefaultChildren = function(registerTypes) {
	if (!registerTypes.includes(PAYPAL_HOSTED_FIELDS_TYPES.NUMBER) || !registerTypes.includes(PAYPAL_HOSTED_FIELDS_TYPES.CVV) || validateExpirationDate(registerTypes)) throw new Error(HOSTED_FIELDS_CHILDREN_ERROR);
};
/**
...
var noDuplicateChildren = function(registerTypes) {
	if (registerTypes.length !== new Set(registerTypes).size) throw new Error(HOSTED_FIELDS_DUPLICATE_CHILDREN_ERROR);
};
/**
...
var PayPalHostedFieldsProvider = function(_a) {
	...
	(0, import_react.useEffect)(function() {
		...
		if (!hostedFields.current) throw new Error(generateMissingHostedFieldsError((_a = { components: options.components }, _a[SDK_SETTINGS.DATA_NAMESPACE] = options[SDK_SETTINGS.DATA_NAMESPACE], _a)));
		if (!hostedFields.current.isEligible()) return setIsEligible(false);
		if (cardFields) cardFields.teardown();
		hostedFields.current.render({
			...
			if (hostedFieldsContainerRef.current) setCardFields(cardFieldsInstance);
		}).catch(function(err) {
			setErrorState(function() {
	...
	return import_react.createElement("div", { ref: hostedFieldsContainerRef }, isEligible ? import_react.createElement(PayPalHostedFieldsContext.Provider, { value: {
		cardFields,
		registerHostedField
...
var PayPalHostedField = function(_a) {
	...
	(0, import_react.useEffect)(function() {
		var _a;
		if (!(hostedFieldContext === null || hostedFieldContext === void 0 ? void 0 : hostedFieldContext.registerHostedField)) throw new Error("The HostedField cannot be register in the PayPalHostedFieldsProvider parent component");
	...
	return import_react.createElement("div", __assign({}, props));
};
/**
...
var usePayPalCardFieldsRegistry = function() {
	var setError = (0, import_react.useState)(null)[1];
	var registeredFields = (0, import_react.useRef)({});
	var registerField = function() {
		var props = [];
		for (var _i = 0; _i < arguments.length; _i++) props[_i] = arguments[_i];
		var fieldName = props[0], options = props[1], cardFields = props[2];
		if (registeredFields.current[fieldName]) setError(function() {
...
var FullWidthContainer = function(_a) {
	...
	return import_react.createElement("div", { style: { width: "100%" } }, children);
};
/**
...
var PayPalCardFieldsProvider = function(_a) {
	...
	(0, import_react.useEffect)(function() {
		...
		if (props.inputEvents) proxyProps.inputEvents = proxyInputEvents;
		try {
		...
		if (!cardFieldsInstance.current) {
			setError(function() {
				var _a;
				throw new Error(generateMissingCardFieldsError((_a = { components: options.components }, _a[SDK_SETTINGS.DATA_NAMESPACE] = options[SDK_SETTINGS.DATA_NAMESPACE], _a)));
			});
			return;
		}
	...
	if (!isEligible) return import_react.createElement("div", null);
	return import_react.createElement(FullWidthContainer, null, import_react.createElement(PayPalCardFieldsContext.Provider, { value: {
		cardFieldsForm,
		fields,
		registerField,
		unregisterField
...
var PayPalCardField = function(_a) {
	...
	(0, import_react.useEffect)(function() {
		...
		if (!containerRef.current) return closeComponent;
		if (options.inputEvents) options.inputEvents = proxyInputEvents;
		var registeredField = registerField(fieldName, options, cardFieldsForm);
		registeredField === null || registeredField === void 0 || registeredField.render(containerRef.current).catch(function(err) {
			if (!hasChildren(containerRef)) return;
			setError(function() {
...
var PayPalNameField = function(options) {
	return import_react.createElement(PayPalCardField, __assign({ fieldName: "NameField" }, options));
};
var PayPalNumberField = function(options) {
	return import_react.createElement(PayPalCardField, __assign({ fieldName: "NumberField" }, options));
};
var PayPalExpiryField = function(options) {
	return import_react.createElement(PayPalCardField, __assign({ fieldName: "ExpiryField" }, options));
};
var PayPalCVVField = function(options) {
	return import_react.createElement(PayPalCardField, __assign({ fieldName: "CVVField" }, options));
};
...
var PayPalCardFieldsForm = function(_a) {
	...
	return import_react.createElement("div", { className }, import_react.createElement(PayPalCardField, { fieldName: "NameField" }), import_react.createElement(PayPalCardField, { fieldName: "NumberField" }), import_react.createElement(FlexContainer, null, import_react.createElement(FullWidthContainer, null, import_react.createElement(PayPalCardField, { fieldName: "ExpiryField" })), import_react.createElement(FullWidthContainer, null, import_react.createElement(PayPalCardField, { fieldName: "CVVField" }))));
};
var FUNDING$1 = {
	PAYPAL: "paypal",
	VENMO: "venmo",
