...
import { getUserAddresses } from '../actions/action';
...
const Checkout = () => {
    const { cart: reduxCart = [], totalPrice: reduxTotalPrice } = useSelector((state) => state.carts);
    const cart = reduxCart.length > 0
        ? reduxCart
        : JSON.parse(localStorage.getItem("cartItems") || "[]");
    const totalPrice = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);

    const [activeStep, setActiveStep] = useState(0); // ← ligne manquante
